...
const CACHE_KEY = 'coingecko_sector_data';
	...
	useEffect(() => {
		const fetchSectors = async () => {
			try {
				...
				// Fetch from Pro API
				const response = await axios.get('https://pro-api.coingecko.com/api/v3/coins/categories', {
					headers: {
					...
					<tbody>
						{loading ? (
							...
							sectors.map((sector, i) => {
								...
								return (
									<tr key={sector.id} className='border-b border-zinc-800'>
										<td className='py-2 pr-3 text-green-400 font-bold'>{i + 1}</td>
										...
										<td className='py-2 pr-3 flex items-center gap-2'>
											{topCoin && (
												<>
													<img src={topCoin} alt='' className='w-5 h-5 rounded-full' />
										...
										<td className='py-2 pr-3'>{formatNumber(sector.market_cap)}</td>
										<td className='py-2 pr-3'>{formatNumber(sector.volume_24h)}</td>
										<td className='py-2'>
